...
export async function gerarDanfsePdf(xml, { cancelada = false } = {}) {
  ...
  // Não é um "aviso de desconfiança" — este DANFSe é tão válido quanto
  // qualquer outro (a API do governo que gerava foi desativada em
  // 01/07/2026, e a partir daí virou responsabilidade de cada software
  // seguir o layout oficial). A autenticidade se confirma pela chave de
  // acesso/QR Code, não por quem desenhou o PDF — mas é justo deixar
  // explícito que o desenho é nosso.
  pagina.drawText("Documento gerado automaticamente pelo iApura a partir do XML oficial da NFS-e.", {
    x: MARGEM,
    y: MARGEM - 10,
    size: 6,
    font: fonte,
